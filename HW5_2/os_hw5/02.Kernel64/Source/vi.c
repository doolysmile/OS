...
void kTest(int XX, int YY, char *vcFileName, int currentEntryIndex){
	FILE* fp;
    int iEnterCount;
    BYTE bKey;
	...
	if(iDirectoryEntryOffset == -1){
		...
		pstFile = fopen( vcFileName, "w", currentEntryIndex);
	...
	kClearScreen();
	kSetCursor(0, 1);
    fp = fopen( vcFileName, "r", currentEntryIndex );
    if( fp == NULL )
    {
			...
			switch(bKey)
			{
				case 10:	//enter
					if(iviBufferIndex >0)
					{
						...
						{
							kSaveFile(vcFileName, currentEntryIndex);
						}
...
void kSaveFile(char *vcFileName, int currentEntryIndex)
{
	int x, y;
	BYTE temp;
    CHARACTER* pstScreen = ( CHARACTER* ) CONSOLE_VIDEOMEMORYADDRESS;
	kGetCursor(&x, &y);
	FILE* fp;
	fp = fopen( vcFileName, "w", currentEntryIndex );
	for(int i = 80; i < 90; i++)
	{
