...
BOOL kFormat( void )
{
         ...
         i++ )
    {
		...
		( ( DWORD* ) ( gs_vbTempBuffer ) )[ i ] = FILESYSTEM_FREECLUSTER;
	...
	DWORD dwCluster = kFindFreeCluster();
   	int dwEntry = kFindFreeDirectoryEntry();
	DIRECTORYENTRY root;

	root.flag = FILESYSTEM_TYPE_DIRECTORY;
	kMemCpy(root.vcFileName, "/", 2);
	root.dwFileSize = 0;
	root.dwStartClusterIndex = 0;
	root.user = 7;
	root.group = 7;
	root.other = 7;
	root.ParentDirectoryEntryIndex = -1;
	kMemSet(root.ParentPath, '\0', 100);
	kReadRTCTime( &(root.bHour), &(root.bMinute), &(root.bSecond) );
	kReadRTCDate( &(root.wYear), &(root.bMonth),
				                &(root.bDayOfMonth), &(root.bDayOfWeek) );
	if(kSetDirectoryEntryData(dwEntry, &root) == FALSE)
		return FALSE;
	kSetClusterLinkData(dwCluster, FILESYSTEM_LASTCLUSTER);

	kSetDotInDirectory(dwEntry);
    kUnlock( &( gs_stFileSystemManager.stMutex ) );
    return TRUE;
}
...
static int kFindFreeDirectoryEntry( void )
{
	...
	DIRECTORYENTRY* tmp = (DIRECTORYENTRY*)gs_vbTempBuffer;
	for(int i = 0 ; i < FILESYSTEM_MAXDIRECTORYENTRYCOUNT ; i++ )
    {
        if( (&tmp[i])->flag == FILESYSTEM_TYPE_FREE )
        {
            return i;
        }
    }
...
static BOOL kSetDirectoryEntryData( int iIndex, DIRECTORYENTRY* pstEntry )
{
	...
	//gs_vbTempBuffer[iIndex] = FILESYSTEM_LASTCLUSTER;
    return TRUE;
}
        ...
        int* piDirectoryEntryIndex, int ParentEntryIndex )
{
    ...
    *piDirectoryEntryIndex = kFindFreeDirectoryEntry();
	...
	pstEntry->ParentDirectoryEntryIndex = ParentEntryIndex;
    pstEntry->flag = FILESYSTEM_TYPE_FILE;
    pstEntry->user = 6;
    pstEntry->group = 6;
    pstEntry->other = 4;
...
FUSER* kOpenUserFile( const char* pcFileName, const char* pcMode ,int count,char* id, char* pwd, int idlen, int pwdlen )
{
    ...
    if( iDirectoryEntryOffset == -1 )
    {
        ...
        // 나머지 옵션들은 파일을 생성
        if( kCreateFile( pcFileName, &stEntry, &iDirectoryEntryOffset, 0 ) == FALSE )
        {
            // 동기화
            kUnlock( &( gs_stFileSystemManager.stMutex ) );
            return NULL;
...
FILE* kOpenFile( const char* pcFileName, const char* pcMode, int ParentEntryIndex )
{
    ...
    if( iDirectoryEntryOffset == -1 )
    {
        ...
        if( kCreateFile( pcFileName, &stEntry, &iDirectoryEntryOffset, ParentEntryIndex ) == FALSE )
        {
            kUnlock( &( gs_stFileSystemManager.stMutex ) );
            return NULL;
        }
...
DIR* kOpenDirectory( const char* pcDirectoryName, int currentDirectoryEntryIndex )
{
    ...
    DIRECTORYENTRY stEntry;
    int iDirectoryEntryOffset, iFileNameLength;
    DWORD dwSecondCluster;
    FILE* pstFile;
	...
	if(kMemCmp(pcDirectoryName, "/", 2) != 0)
	{
        if( iDirectoryEntryOffset == -1)
        {
            if( kCreateDirectory( pcDirectoryName, &stEntry, &iDirectoryEntryOffset, currentDirectoryEntryIndex ) == FALSE )
            {
                kUnlock( &( gs_stFileSystemManager.stMutex ) );
                return NULL;
            }
    ...
    if( kReadCluster( currentEntryIndex, ( BYTE* ) pstDirectoryBuffer ) == FALSE )
    {
        kFreeFileDirectoryHandle( pstDirectory );
...
{
    DWORD dwCluster;
	DIRECTORYENTRY* rootdir = kFindDirectory(0);
    ...
    *piDirectoryEntryIndex = kFindFreeDirectoryEntry();
	...
	pstEntry->flag = FILESYSTEM_TYPE_DIRECTORY;
	kMemCpy( pstEntry->vcFileName, pcFileName, kStrLen( pcFileName )+1 );
    pstEntry->dwStartClusterIndex = dwCluster;
    pstEntry->dwFileSize = 0;
    pstEntry->ParentDirectoryEntryIndex = ParentEntryIndex;
	kMemSet(pstEntry->ParentPath, 0, 100);
	kMemCpy(pstEntry->ParentPath, (&rootdir[ParentEntryIndex])->ParentPath,
			kStrLen((&rootdir[ParentEntryIndex])->ParentPath));
	kMemCpy((pstEntry->ParentPath)+kStrLen(pstEntry->ParentPath),
			(&rootdir[ParentEntryIndex])->vcFileName,
			kStrLen((&rootdir[ParentEntryIndex])->vcFileName)+1);
	if(ParentEntryIndex != 0)
		kMemCpy((pstEntry->ParentPath)+kStrLen(pstEntry->ParentPath), "/", 2);
	pstEntry->user = 7;
    pstEntry->group = 7;
    pstEntry->other = 5;        //default = 755;
    kReadRTCTime( &(pstEntry -> bHour), &(pstEntry -> bMinute),
			&(pstEntry -> bSecond) );
    kReadRTCDate( &(pstEntry -> wYear), &(pstEntry -> bMonth),
			&(pstEntry -> bDayOfMonth), &(pstEntry -> bDayOfWeek) );
    ...
    kSetDotInDirectory(*piDirectoryEntryIndex);
    return TRUE;
}
/** 
 * 디렉토리의 dot, dot dot 생성
 */
void kSetDotInDirectory(int ParentEntryIndex ){
    DIRECTORYENTRY	stEntry, pstEntry;
	int				dwEntry = kFindFreeDirectoryEntry();
	DIRECTORYENTRY* rootdir = kFindDirectory(0);
	
	stEntry.flag = FILESYSTEM_TYPE_DIRECTORY;
	kMemCpy( stEntry.vcFileName, ".", 2 );
	stEntry.dwStartClusterIndex = (&rootdir[ParentEntryIndex])->dwStartClusterIndex;
	kReadRTCTime( &(stEntry.bHour), &(stEntry.bMinute), &(stEntry.bSecond) );
	kReadRTCDate( &(stEntry.wYear), &(stEntry.bMonth),
				            &(stEntry.bDayOfMonth), &(stEntry.bDayOfWeek) );
	stEntry.ParentDirectoryEntryIndex = ParentEntryIndex;
    if( kSetDirectoryEntryData( dwEntry, &stEntry ) == FALSE )
        return FALSE;
//==================================================================================
//==================================================================================
	dwEntry = kFindFreeDirectoryEntry();

	pstEntry.flag = FILESYSTEM_TYPE_DIRECTORY;
	kMemCpy( pstEntry.vcFileName, "..", 3 );
	if(ParentEntryIndex == 0)
		pstEntry.dwStartClusterIndex = -1;
	else
		pstEntry.dwStartClusterIndex = (&rootdir[(&rootdir[ParentEntryIndex])->ParentDirectoryEntryIndex])->dwStartClusterIndex;
	kReadRTCTime( &(pstEntry.bHour), &(pstEntry.bMinute), &(pstEntry.bSecond) );
	kReadRTCDate( &(pstEntry.wYear), &(pstEntry.bMonth),
			&(pstEntry.bDayOfMonth), &(pstEntry.bDayOfWeek) );
	pstEntry.ParentDirectoryEntryIndex = ParentEntryIndex;
	if( kSetDirectoryEntryData( dwEntry, &pstEntry ) == FALSE )
        return FALSE;
}

void kSetEntryIndex(DWORD currentDirectoryEntryIndex)
{
    currentEntryIndex = currentDirectoryEntryIndex;
}
 ...
 DIRECTORYENTRY* kFindDirectory( int iIndex )
{
    ...
    DIRECTORYENTRY* pstEntry = ( DIRECTORYENTRY* ) &(gs_vbTempBuffer[iIndex]);
    if(pstEntry == NULL)
        return NULL;
    return pstEntry;
}
...
BOOL kUpdateDirectory( int piDirectoryEntryIndex,const char* fileName,const char* parentPath, int parentIndex )
{
    ...
    stEntry.dwFileSize = 0;
    stEntry.flag=FILESYSTEM_TYPE_DIRECTORY;
    stEntry.ParentDirectoryEntryIndex = parentIndex;
